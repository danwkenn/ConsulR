...
#' @param file R-script file to be checked.
...
check_file_runs <- function(file,verbose = FALSE,line_selection = NULL){

  template_location <- system.file("templates", package = "ConsulR")
  template_file <- paste0(template_location,"/template-check-run.R")
  temp_dir <- tempdir()
  ...
  test_file <- stringr::str_replace(template,"%FILENAME%",paste0("\\\"",file,"\\\""))
